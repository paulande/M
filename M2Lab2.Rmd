...
Unlike the built-in R function, this function has a "smooth" argument, which adds a tiny positive value to every cell in the table. For tables with all non-zero counts, this changes the chi-squared statistic barely at all.  The smoothing is included to ensure that the chi-squared statistic can always be defined, even if one the rows or one of the columns of a table contains all zeroes.  This is a somewhat pathological situation in practice -- it can occur when we know that there was an additional possible level of a categorical variable, but we happened to not observe that level in the particular sample we have -- but when we run a simulation that resamples thousands of times from a table of small counts, such a situation will not-infrequently arise.      
 ...
 hi^2$ distribution. When the sample size is not large, there is another option for evaluating statistical independence. 
...
str(UCBAdmissions)
  ...
  (UCBA_sum <- margin.table(UCBAdmissions,c(1,2))) # keep the 1st and 2nd dimension, and sum over the 3rd - flatten
  chisq.test(UCBA_sum) # conduct a chi^2 test on this flattened data
  # note: using apply is a little clearer on what operations you want to use on the collapsed dimention apply(my_array, c(1, 2), sum)
```
...
Note: H_0 in this case is D1 and D2 ARE independent

Note: Alignment in a mosaic plot > dimensions ARE independent, misalignment of dimension ARE NOT independent
...
```{r}
  chisq.test(UCBA_sum) # recommendation is to always use Yates' continuity correction, which is the default
...
The continuity correction was introduced to accommodate the use of a continuous probability distribution (the $\chi^2$ distribution) as the reference distribution for a statistic calculated from non-continuous (i.e., categorical) data. Simply put, it makes the performance of the $\chi^2$ statistic better (in terms of type II error - false neg) when the sample size is of moderate size.  We recommend that you simply use the default, `correct = TRUE`.

Note: recall that chi^2 test = test of homogeneity

The $\chi^2$ test is also called a test for homogeneity in some situations, and the `UCBAdmissions` dataset provides a good example. Another way that we could consider the independence (or lack of association) between admission and gender is to ask whether the proportion of male applicants admitted is the same as the proportion of female applicants admitted. Put another way, we ask whether the proportions of admits are the same (homogeneous) between the two groups, male applicants and female applicants:

Note: for two groups H_0: prop A = prop B; p is homogeneous between the two groups; Q chi^2 can answer = is there a dif?

Note: This sounds a lot like the case in which we would normally use a t test. So recall:
Test	      Data Type	                            Question
Chi-squared	Categorical vs. Categorical	          Are the proportions different?
t-test	    Categorical (grouping) vs. Continuous	Are the means different?

Note: Dimensions of a matrix
[Admit, Gender, Dept]
[rows, columns, layers]
  1      2        3
  
Therefore, when we transpose, we "pivot" the table. In the case below, Gender is now in the 1st dimension spot and admit is in the 2nd.

```{r}
# print the original matrix
UCBA_sum

# transpose the UCBA_sum matrix, so Gender is the row variable:
  (UCBA_sum2 <- t(UCBA_sum))

# run the chi^2 on original
chisq.test(UCBA_sum)
...
Note: Collapsing "flattening" is aggregation, use caution.

```{r}
(UCBApplicants <- margin.table(UCBAdmissions, c(2,3))) # collapsing over admit, by sum
...
Note: Chi^2 test = Dif in Proportion Tests
...
> There's also a good book called, not surprisingly, "The Lady Tasting Tea," by David Salsburg that gives an entertaining early history of Statistics. [I, Kollin, can neither confirm nor deny this statement which comes from a previous instructor.]

The following R chunk recreates the Lady Tasting Tea data. "confusion matrix"

Note: By adding the 2nd dimention, we see the information not just for wrong and right, but what the correct answer was for each answer. Interesting.
...
We now present a simulation that should give you some insight into the problem here. Small sample size?
...
Note: ref dist from resampling CLOSE true ref dist of chi^2 stat
...
Note: one fixed margin (true) 4 of each; one unfixed (guess; subject doesn't know)
Note distribution?: T/F independent multinomials (rows OR columns)
...
Note: both fixed; truth is fixed, subject knows
Dist: hypergeometric distribution (generalized)
...
ggplot() +
  ...
  geom_line(mapping = aes(x = chisq_val, y = chisq_theory, color = "Theoretical Chi-sq")) +
  geom_density(mapping = aes(x = resamp_tea$stats, color = "Resampled Chi-sq"), bw = 0.05) + 
  geom_vline(aes(xintercept = chisq_stat0(TeaTasting), color = "Observed Stat")) +
  guides(color = guide_legend(title = "Legend"))
```

In this plot, the smooth blue line is the theoretical Chi-squared (df = 1) reference distribution. The [green] line with a few spikes is the reference distribution we get by running the re-sampling code. There are actually only a few values of the chi-squared statistic that are possible with such a sparse table. Finally, the vertical red line is the value of the chi-squared statistic calculated from the observed table. 
...
This is precisely why using the large sample reference distribution (blue line) isn't such a good idea here, because is does a really poor job of approximating the resampled reference distribution ([green] line).
...
Note: Fisher's Exact Test: tests if two categorical vars are independent; test of independence
Note: used - small sampled sizes, assumes both are FIXED, uses odds ratio test
Note: p-value 0.4857 we fail to reject H_0
Note: H_0: p guesses = p random or odds ration = 1; by rejecting H_0 - no strong evid the guesses were better than random
...
Note: CI is extremely wide b/c sample size is small
Note: CI includes 1, no effect
...
Note: equal odds ratios = equal proportions = homogeneity
Note: but choose which based on your data
...
Note: these tests only result in independence verification
